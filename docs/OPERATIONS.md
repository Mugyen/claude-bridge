...
- **`git pull` does NOT update a running bridge.** The server loads `bridge-server.mjs` once at startup — after pulling a *server* change, **restart the bridge on that node** (`./claude-bridge --restart`, or kill the listener + `--start`). Hooks are the exception: they're re-read on every invocation, so hook fixes go live on pull alone. Each machine has its own checkout, so `git pull` on **every** node, then restart its bridge. Symptom of a stale server: it broadcasts an old roster — cross-node **descriptions leak** and remote sessions are **mis-tagged `local`** (OPS-4).
...
./claude-bridge                 # hooks + MCP + skill
./claude-bridge --start         # start the bridge on :7400
# from a SEPARATE terminal:
./claude-bridge --share                         # quick tunnel (ephemeral *.trycloudflare.com URL)
#   OR a stable named tunnel (see §4):
./claude-bridge --share --named-tunnel <your-host>
./claude-bridge --check         # role=hub, fed port, tunnel URL + join link
```
...
git checkout feature/cross-network-federation
./claude-bridge
./claude-bridge --join 'https://<host>#<TOKEN>'   # paste the hub's exact join link
./claude-bridge --restart                          # needed today (token changed → BUG-3)
./claude-bridge --check                            # role=spoke, hub=<host>, link connected
```
...
cloudflared tunnel create <name>         # → ~/.cloudflared/<UUID>.json ; name it = the hostname for claude-bridge's --named-tunnel to work
...
```bash
# claude-bridge way (tunnel NAME must equal the hostname):
./claude-bridge --share --named-tunnel <host>
#   ⚠️ if a tunnel is already open, run ./claude-bridge --stop-share FIRST (BUG-4)
...
## 5. Verify / inspect
```bash
./claude-bridge --check                                   # role, ports, tunnel, link
...
## 6. Teardown
```bash
# spoke:  ./claude-bridge --unlink
# hub:    ./claude-bridge --stop-share        # closes tunnel, keeps token for fast re-share
# stop the bridge (from a non-bound terminal):  ./claude-bridge --stop
```
...
| hub rejects the correct token (`401`) | running process has an old token; reload was dropped (BUG-3) | `./claude-bridge --restart` |
| dotfiles say spoke but it's still a hub | token-change reload silently failed (BUG-3) | `./claude-bridge --restart` |

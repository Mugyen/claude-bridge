...
### 36. Room-first UX: `share`/`stop-share`/`unlink` are GONE — room verbs own connectivity

v2.10.0 removed the user-facing `share`/`stop-share`/`unlink` commands. `room start`/`room stop`/`room leave` are the surface; internally they still call `share_hub`/`stop_share`/`unlink_hub` (the provider/tunnel mechanism is unchanged — `share_hub` gained a `ROOM_MODE=1` guard that suppresses the bare-token join-link print, since a room member authenticates with a per-member token, NOT the bridge token). `room start` auto-creates a default password room if none exists; password is ON by default (`--open` opts out → server allows credential-less join when `room.open`). Connectivity + the rendezvous code are lifecycle-managed: start/create publish the code, stop/delete `release_code` it. Two `set -e` traps bit us and are fixed: (a) `release_code` must `[ -s "$CODES_FILE" ] || return 0` and `|| true` its jq (a failing jq in `$(...)` aborts the caller SILENTLY — swallowed `room delete`'s output); (b) `expiry_human` must validate pure-int before `$(( ))` (the security fix). Tests that drove `share`/`stop-share` now drive `room start`/`room stop` and MUST isolate `CC_BRIDGE_RENDEZVOUS` (→ a dead local port) + `CC_BRIDGE_CODES_FILE`/`CC_BRIDGE_ROOMS_FILE` — otherwise `room start`'s auto-publish hits the LIVE `r.houserbot.com`. `test-share-flags.sh` was retired (coverage moved to test-providers/test-room-ux). The rendezvous Worker is deployed and live at r.houserbot.com (Cloudflare account b24hoover@gmail.com).

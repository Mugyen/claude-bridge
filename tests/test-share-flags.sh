#!/bin/bash
# install.sh federation flags: --join parses a link into the right files,
...
# Isolated fake HOME so install.sh writes config under a temp dir.
TMP=$(mktemp -d)
...
CC_BRIDGE_PORT=$PORT bash "$REPO_DIR/install.sh" --join 'https://calm-river-1234.trycloudflare.com#secrettoken99' >/dev/null 2>&1 || true
